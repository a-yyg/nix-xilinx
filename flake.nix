{
  ...
  let
    ...
    xsctPaths = [
      "gnu/aarch32/lin/gcc-arm-none-eabi/bin"
      "gnu/aarch32/lin/gcc-arm-linux-gnueabi/bin"
      "gnu/aarch64/lin/aarch64-none/bin"
      "gnu/aarch64/lin/aarch64-linux/bin"
      "gnu/armr5/lin/gcc-arm-none-eabi/bin"
      "gnu/microblaze/lin/bin"
      "components/xsct/bin"
      "components/xsct/bin/scripts"
    ];
    defaultPetalinuxEnv = ''
      ...
      ${pkgs.lib.concatMapStringsSep "\n" (path: ''PATH="$XSCT_TOOLCHAIN/${path}:$PATH"'') xsctPaths}
      "$PETALINUX/scripts/bash/petalinux-env-check"
    '';
    runScriptPrefix = { errorOut ? true, petalinux ? false, petalinuxEnv ? defaultPetalinuxEnv }: let
    ...
    '' + /*
        ...
        */ pkgs.lib.optionalString petalinux petalinuxEnv
    ) + ''
      fi
    '';
  ...
  in {
    ...
    packages.x86_64-linux.petalinux = pkgs.buildFHSEnvChroot {
      ...
      runScript = pkgs.writeShellScript "petalinux-wrapper" ((runScriptPrefix {
        ...
        exec "$PETALINUX/scripts/petalinux-$NIX_PETALINUX_TOOL" "$@"
      '');
      extraInstallCommands = pkgs.lib.pipe [
        "boot" "build" "config" "create" "devtool" "package" "upgrade" "util"
      ] [
        (map (t: ''
          cp $out/bin/petalinux $out/bin/petalinux-${t}
          wrapProgram $out/bin/petalinux-${t} \
